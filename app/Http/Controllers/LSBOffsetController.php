...
class LSBOffsetController extends Controller
{
    public function showOffsetLSB()
    {
        return view('lsb_offset');
    }

    public function LSBOffsetEncode(Request $request)
    {
        ...
        for ($x = 0; $x < $x_dimension; $x++) {
            ...
            for ($y = 0; $y < $y_dimension; $y++) {
                ...
                $blueBinaryArray[count($blueBinaryArray)-1] = $binaryText[$count-$offset];
    ...
    public function LSBOffsetDecode(Request $request)
    {
        $pictures = $request->get('pictures');
        $original = preg_replace('/data:image\/\w+;base64,/', '', $pictures['original']);
        $original = base64_decode($original);
